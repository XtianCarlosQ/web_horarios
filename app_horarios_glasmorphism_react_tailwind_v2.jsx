...
export default function App() {
  ...
  // Parsear texto OCR → curso
  const parseCourseFromText = (text) => {
    const norm = text.replace(/\u00A0/g, ' ').replace(/\s+/g, ' ').trim();

    // Nombre (línea con "Edic" o la línea más larga)
    let name = null;
    const edicMatch = norm.match(/\b\d+\w*\s*Edic\.?\s*([^\n]+)/i);
    if (edicMatch) name = edicMatch[1].trim();
    if (!name) {
      const lines = text.split(/\n+/).map((s) => s.trim()).filter(Boolean);
      name = lines.sort((a,b)=>b.length-a.length)[0]?.slice(0,120) || "Curso nuevo";
    }

    // Fecha de inicio
    const startMatch = norm.match(/Inicio\s*:?\s*(\d{4}-\d{2}-\d{2})/i);
    const startDate = startMatch ? startMatch[1] : new Date().toISOString().slice(0,10);

    // Horas totales
    const hrsMatch = norm.match(/(Cantidad de Horas|Horas|Duración)[^\d]*(\d{1,2})\s*h?r?s?/i);
    const hoursTotal = hrsMatch ? parseInt(hrsMatch[2],10) : 16;

    // Sesiones por día
    const sessions = [];
    const regex = /(Lunes|Martes|Miércoles|Jueves|Viernes|Sábado|Domingo)[^\d]*(\d{1,2}:\d{2})\s*(?:-|–|a|al|\–)\s*(\d{1,2}:\d{2})/gi;
    let m;
    while ((m = regex.exec(text)) !== null) {
      const day = m[1];
      const s = m[2];
      const e = m[3];
      const fix = (t)=> t.length===4?`0${t}`:t;
      sessions.push({ day, start: fix(s), end: fix(e) });
    }
    if (sessions.length===0) {
      // fallback si OCR rompió acentos
      const regex2 = /(Lunes|Martes|Miercoles|Jueves|Viernes|Sabado|Domingo)[^\d]*(\d{1,2}:\d{2}).*?(\d{1,2}:\d{2})/gi;
      while ((m = regex2.exec(text)) !== null) {
        let day = m[1].replace('Miercoles','Miércoles').replace('Sabado','Sábado');
        const s = m[2], e = m[3];
        sessions.push({ day, start: s, end: e });
      }
    }

    return { name, startDate, hoursTotal, sessions };
  };

  // Procesar imagen con Tesseract y añadir curso
  const addCourseFromImage = async (file) => {
    if (!file) return;
    setOcrProgress(0);
    ...
    reader.onload = async () => {
      ...
      try {
        ...
        }).catch(()=> Tesseract.recognize(dataUrl, 'eng'));
        const parsed = parseCourseFromText(data.text || '');
        const newId = `c-${Date.now()}`;
        const color = COLOR_PALETTE[(Math.random()*COLOR_PALETTE.length)|0];
        const course = { id: newId, name: parsed.name, color, enabled: true, startDate: parsed.startDate, hoursTotal: parsed.hoursTotal, sessions: parsed.sessions };
        setCourses((prev)=> [course, ...prev]);
        setShowAdd(false); setAddingImg(null); setOcrProgress(null);
      } catch (e) {
        console.error(e);
        setOcrProgress(null);
        alert('No pude extraer los textos del pantallazo. Intenta con otra imagen o ajusta manualmente.');
      }
    };
      ...
      <main className="mx-auto max-w-[1300px] px-6 pb-28 pt-10">
        ...
        {showAdd && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 p-4">
            <div className="w-full max-w-2xl rounded-3xl border border-white/10 bg-slate-900/80 p-6 backdrop-blur-xl">
              <div className="mb-4 flex items-center justify-between">
                <h3 className="text-lg font-semibold">Agregar curso desde imagen</h3>
                <button onClick={() => { setShowAdd(false); setAddingImg(null); setOcrProgress(null); }} className="rounded-xl bg-white/10 px-3 py-1 text-sm hover:bg-white/15">Cerrar</button>
              </div>

              <div
                onDragOver={(e)=>{e.preventDefault();}}
                onDrop={(e)=>{e.preventDefault(); const f = e.dataTransfer.files?.[0]; if (f) addCourseFromImage(f);}}
                className="group grid place-items-center rounded-2xl border border-dashed border-white/20 p-8 text-center hover:border-white/40"
              >
                <input id="fileInput" type="file" accept="image/*" className="hidden" onChange={(e)=>{const f=e.target.files?.[0]; if(f) addCourseFromImage(f);}} />
                <div>
                  <p className="text-slate-300">Suelta aquí una captura de curso o </p>
                  <button onClick={()=>document.getElementById('fileInput').click()} className="mt-2 rounded-xl bg-white/10 px-4 py-2 text-sm hover:bg-white/15">Elegir archivo</button>
                </div>
              </div>

              {ocrProgress!==null && (
                <div className="mt-4">
                  <div className="mb-1 text-xs text-slate-400">Extrayendo texto… {ocrProgress}%</div>
                  <div className="h-2 w-full overflow-hidden rounded-full bg-white/10">
                    <div className="h-full" style={{ width: `${ocrProgress}%` }} />
                  </div>
                </div>
              )}
            </div>
